    ...
    public class PaintNewspaperDao
    {
        ...
        public bool Exanmie ( LineProductMesEntityu . PaintNewspaperHeaderEntity  model )
        {
            ...
            if ( model . PAN009 )
            {
                ...
                strSql . AppendFormat ( "SELECT A.PAO002 ANN002,A.PAO003 ANN003,PAO008 ANN005,PAO012 ANN009,DDA001,CEILING(RAB)-SUM(ISNULL(RCC006,0)) RCC FROM MIKPAO A LEFT JOIN TPADEA B ON A.PAO003=B.DEA001 INNER JOIN TPADDA C ON B.DEA008=C.DDA001 INNER JOIN (SELECT PAO002,PAO003,MAX(CASE WHEN RAB007=0 OR RAA018=0 THEN 0 ELSE RAB008/(RAB007/RAA018) END) RAB FROM MIKPAO A INNER JOIN SGMRAA F ON A.PAO002=F.RAA001 AND A.PAO003=F.RAA015 INNER JOIN SGMRAB G ON F.RAA001=G.RAB001 WHERE PAO001='{0}' GROUP BY PAO002,PAO003) H ON A.PAO002=H.PAO002 AND A.PAO003=H.PAO003 LEFT JOIN SGMRCC E ON A.PAO001=E.RCC002 AND A.PAO002=E.RCC004 AND A.PAO003=E.RCC010 WHERE PAO001='{0}' GROUP BY A.PAO002,A.PAO003,PAO008,PAO012,DDA001,RAB,PAO001" ,model . PAN001 );
