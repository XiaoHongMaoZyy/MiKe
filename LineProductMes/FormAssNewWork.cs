...
namespace LineProductMes
{
    public partial class FormAssNewWork :FormChild
    {
        ...
        DataTable tableWork,tableView,tableViewTwo,tableOtherSur,tableUser,tablePInfo,talePrintOne,talePrintTwo,tablePrintTre,tablePrintFor,tablePrintFiv,getUseForEntry;
        DataRow row;

        List<string> idxDelete=new List<string>();
        List<string> idxDeleteOne=new List<string>();

        //int selectIndex;
        string parame=string.Empty,strWhere=string.Empty,state=string.Empty,focusName=string.Empty;
        bool result=false;
        decimal entrySala=0M;
        ...
        protected override int Examine ( )
        {
            ...
            if ( _header . ANW020 && checkDataConsistency ( ) == false )
                return 0;
        ...
        private void gridControl2_KeyPress ( object sender ,KeyPressEventArgs e )
        {
            if ( toolSave . Visibility == DevExpress . XtraBars . BarItemVisibility . Always && e . KeyChar == ( char ) Keys . Enter )
            {
                ...
                if ( XtraMessageBox . Show ( "确认删除?" ,"提示" ,MessageBoxButtons . YesNo ) == DialogResult . Yes )
                {
                    ...
                    if ( _bodyOne . idx > 0 && !idxDeleteOne . Contains ( _bodyOne . idx . ToString ( ) ) )
                        idxDeleteOne . Add ( _bodyOne . idx . ToString ( ) );
        ...
        private void bandedGridView1_CellValueChanged ( object sender ,DevExpress . XtraGrid . Views . Base . CellValueChangedEventArgs e )
        {
            ...
            {
                if ( row [ "ANX011" ] != null && row [ "ANX011" ] . ToString ( ) != string . Empty && ( row [ "ANX011" ] . ToString ( ) . Equals ( "离职" ) || row [ "ANX011" ] . ToString ( ) . Equals ( "未上班" ) || row [ "ANX011" ] . ToString ( ) . Equals ( "请假" ) ) )
                {
        ...
        private void txtANW015_EditValueChanged ( object sender ,EventArgs e )
        {
            ...
            if ( string . IsNullOrEmpty ( txtANW015 . Text ) )
                return;
            getUseForEntry = _bll . getUseForEntry ( Convert . ToDateTime ( txtANW015 . Text ) );
        }
        ...
        void updateBatchTime ( )
        {
            ...
            foreach ( DataRow row in tableView . Rows )
            {
                if ( txtANW014 . Text . Equals ( "计件" ) )
                {
                    if ( row [ "ANX011" ] . ToString ( ) . Equals ( "在职" ) )
                    {
                        row [ "ANX005" ] = dtStart;
                        row [ "ANX006" ] = dtEnd;
                    }
        ...
        void controlUnEnable ( )
        {
            ...
            idxDelete = new List<string> ( );
            idxDeleteOne = new List<string> ( );
        }
        void controlEnable ( )
        {
        ...
        /// <summary>
        /// 入职补贴的工资算到补贴工资中，参与全员工资分配   仅计件
        /// </summary>
        void calcuSalaryForEntry ( )
        {
            if ( "计时" . Equals ( txtANW014 . Text ) )
                return;
            if ( getUseForEntry == null || getUseForEntry . Rows . Count < 1 )
                return;
            ...
            entrySala = 0M;
            DataRow [ ] rows;
            foreach ( DataRow row in getUseForEntry . Rows )
            {
                _body . ANX002 = row [ "EMP001" ] . ToString ( );
                rows = tableView . Select ( "ANX002='" + _body . ANX002 + "'" );
                if ( rows . Length > 0 )
                {
                    _body . ANX011 = rows [ 0 ] [ "ANX011" ] . ToString ( ) . Trim ( );
                    if ( !string . IsNullOrEmpty ( _body . ANX011 ) && ( "请假" . Equals ( _body . ANX011 ) || "在职" . Equals ( _body . ANX011 ) ) )
                    {
                        _body . ANX015 = string . IsNullOrEmpty ( rows [ 0 ] [ "ANX015" ] . ToString ( ) ) == true ? 0 : Convert . ToDecimal ( rows [ 0 ] [ "ANX015" ] );
                        _body . ANX016 = string . IsNullOrEmpty ( rows [ 0 ] [ "ANX016" ] . ToString ( ) ) == true ? 0 : Convert . ToDecimal ( rows [ 0 ] [ "ANX016" ] );
                        entrySala += Convert . ToDecimal ( _body . ANX015 + _body . ANX016 ) * 7;
                    }
                }
            }

            txtu2 . Text = string . IsNullOrEmpty ( txtu2 . Text ) == true ? 0 . ToString ( ) : ( Convert . ToDecimal ( txtu2 . Text ) + entrySala ) . ToString ( "0.######" );
        }
        /// <summary>
        /// 总工资
        /// </summary>
        ...
        void setCalcuSalary ( )
        {
            ...
            if ( "计件" . Equals ( txtANW014 . Text ) )
            {
                ...
                resultSalary = ( string . IsNullOrEmpty ( txtu4 . Text ) == true ? 0 : Convert . ToDecimal ( txtu4 . Text ) + entrySala - ( string . IsNullOrEmpty ( txtu3 . Text ) == true ? 0 : Convert . ToDecimal ( txtu3 . Text ) ) );
        ...
        bool getValue ( )
        {
            ...
            if ( "计件" . Equals ( txtANW014 . Text ) )
            {
                ...
                for ( int i = 0 ; i < gridView1 . RowCount ; i++ )
                {
                    ...
                    if ( row [ "ANN008" ] == null || row [ "ANN008" ] . ToString ( ) == string . Empty || Convert . ToDecimal ( row [ "ANN008" ] ) <= 0 )
                    {
                        row . SetColumnError ( "ANN008" ,"必须大于0    " );
                        result = false;
                        break;
                    }
                }
            ...
            for ( int i = 0 ; i < bandedGridView1 . RowCount ; i++ )
            {
                ...
                if ( row [ "ANX011" ] == null || row [ "ANX011" ] . ToString ( ) == string . Empty )
                {
                    row . SetColumnError ( "ANX011" ,"不可为空" );
                    result = false;
                    break;
                }
                if ( row [ "ANX002" ] == null || row [ "ANX002" ] . ToString ( ) == string . Empty )
                {
                    row . SetColumnError ( "ANX002" ,"不可为空" );
                    result = false;
                    break;
                }
                _body . ANX003 = row [ "ANX003" ] . ToString ( );
                _body . ANX002 = workShopTime . checkWhetherOrNotSameDay ( txtANW015 . Text ,row [ "ANX005" ] . ToString ( ) );
                if ( _body . ANX002 != null )
                {
                    XtraMessageBox . Show ( _body . ANX003 + "计件开工" + _body . ANX002 ,"提示" );
                    result = false;
                    break;
                }
                _body . ANX002 = workShopTime . checkWhetherOrNotSameDay ( txtANW015 . Text ,row [ "ANX006" ] . ToString ( ) );
                if ( _body . ANX002 != null )
                {
                    XtraMessageBox . Show ( _body . ANX003 + "计件完工" + _body . ANX002 ,"提示" );
                    result = false;
                    break;
                }
                _body . ANX002 = workShopTime . checkWhetherOrNotSameDay ( txtANW015 . Text ,row [ "ANX007" ] . ToString ( ) );
                if ( _body . ANX002 != null )
                {
                    XtraMessageBox . Show ( _body . ANX003 + "计时开工" + _body . ANX002 ,"提示" );
                    result = false;
                    break;
                }
                _body . ANX002 = workShopTime . checkWhetherOrNotSameDay ( txtANW015 . Text ,row [ "ANX008" ] . ToString ( ) );
                if ( _body . ANX002 != null )
                {
                    XtraMessageBox . Show ( _body . ANX003 + "计时完工" + _body . ANX002 ,"提示" );
                    result = false;
                    break;
                }
            }
            ...
            if ( ( Convert . ToDateTime ( _header . ANW016 ) - Convert . ToDateTime ( _header . ANW015 ) ) . Days > 0 )
            {
                XtraMessageBox . Show ( "开完工时间不允许跨天" );
                return false;
            }

            if ( _header . ANW026 > 0 && _header . ANW027 > 0 && string . IsNullOrEmpty ( _header . ANW017 ) )
            {
        ...
        bool checkDataConsistency ( )
        {
            ...
            strWhere = "1=1";
            strWhere += " AND ANX001='" + txtANW001 . Text + "'";
            ...
            for ( int i = 0 ; i < bandedGridView1 . RowCount ; i++ )
            {
                ...
                if ( row == null )
                    continue;
                _body . ANX002 = row [ "ANX002" ] . ToString ( );
                _body . ANX017 = string . IsNullOrEmpty ( row [ "ANX017" ] . ToString ( ) ) == true ? 0 : Convert . ToDecimal ( row [ "ANX017" ] );
                if ( ViewOne . Select ( "ANX002='" + _body . ANX002 + "' AND ANX017='" + _body . ANX017 + "'" ) . Length < 1 )
                {
                    XtraMessageBox . Show ( "工号:" + _body . ANX002 + "的工资与后台不一致,请重新编辑保存" );
                    result = false;
                    break;
                }
            }
